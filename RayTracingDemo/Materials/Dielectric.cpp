...
Dielectric::Dielectric(const Dielectric& rm)
	...
	if (rm.fresnel_brdf)
		fresnel_brdf = rm.fresnel_brdf->clone();
	else
		fresnel_brdf = NULL;

	if (rm.fresnel_btdf)
		fresnel_btdf = rm.fresnel_btdf->clone();
	else
		fresnel_btdf = NULL;
}
...
Dielectric::shade(ShadeRec& sr)
{
	...
	else
	{
		...
		if (ndotwi < 0.0)
		{
			...
			Lr = fr * sr.w.tracer_ptr->trace_ray(reflected_ray, t, sr.depth + 1)
				*fabs(ndotwi);
			L += cf_in.powc(t)*Lr; // inside filter color

			//transmitted ray is outside

			Lt = ft* sr.w.tracer_ptr->trace_ray(transmitted_ray, t, sr.depth + 1)
				*fabs(ndotwt);
